 ...
 *
 * Score (0–100) = vulnerabilidad × 100
 *
 ...
 *   rotStress   — riesgo de pudricón por exceso de agua (solo frio-humedo)
 *
...
export interface RiesgoGrupo {
  ...
  plantasGrupo:    Array<{ id: string; nombre: string }>;
}

// ── Main export ──────────────────────────────────────────────────────────────────────────────────
export function calcularRiesgosTienda(
  zona: ZonaClimatica,
  plantasDeTienda: Array<{ id: string; nombre: string; grupo: string }>,
  ...
  for (const [grupo, plantas] of porGrupo.entries()) {
    ...
    // ── Final score 0–100 ─────────────────────────────────────────────────────────────────────
    const puntuacion = Math.round(vulnerabilidad * 100);
